...
- [ ] Build `AskAboutMe.tsx`: RAG route handler + small embedded corpus + retrieval + source-grounding guardrail + typed-out terminal response, per §6. Ship the simple fixed-corpus v1 if time is short — don't skip it entirely.
...
**What it is:** a terminal-styled panel (second use of the terminal motif from §2) where a visitor types a question and gets a grounded answer pulled from your actual resume, project READMEs, and case-study text — using the same retrieval + reranking + guardrail pattern you already built for DoCopilot.

**Architecture (reuse DoCopilot's pattern at a smaller scale):**
- **Corpus:** resume text, the four featured project write-ups, and the archive one-liners — embedded once at build time. This corpus is small enough that you don't need a hosted vector DB for it; a JSON file of embeddings checked into the repo, or a lightweight Qdrant collection if you want architectural parity with DoCopilot, both work. Pick whichever is less operational overhead for a portfolio (a static JSON embedding file is the pragmatic choice — no DB to keep alive).
- **Retrieval:** same hybrid approach (keyword + dense) with reranking if you want full parity with DoCopilot; for a corpus this small, straight dense retrieval + a rerank step over the top-k is enough and keeps it simple to maintain.
- **Guardrails (the part that matters most for credibility):** refuse or redirect off-topic questions ("what's the weather" → "I can only answer questions about Samarth's work"), and **cite which project/section the answer is grounded in** — the same source-grounding check from DoCopilot's guardrail set. This is what separates "working RAG demo" from "chatbot skin."
- **Serving:** a small API route (Next.js route handler) calling your model of choice — doesn't need to be GPT-5-class; a smaller/cheaper model is fine since retrieval is doing the heavy lifting, and it keeps your own hosting costs near zero.
...
**Fallback / scope note:** if time is tight before placements, ship a v1 with a fixed, small FAQ-style corpus and straightforward retrieval (no reranking) rather than skipping it — a simple working version beats a more sophisticated one that doesn't ship before mid-August.
